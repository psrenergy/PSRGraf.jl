...
function PSRGrafBinary.open(
    ...
    if initial_stage != Dates.Date(1900, 1, 1)
        ...
        relative_first_stage = _stage_from_date(initial_stage, stage_type, _date)
...
function _get_hour_discretization(stage_type::StageType, block_total::Integer)
    hour_total = if stage_type == STAGE_WEEK
        168
            ...
            if graf.stage_current < graf.stage_total
                ...
                if graf.hours_exist
                    graf.block_total_current =
                        blocks_in_stage(graf, graf.stage_current)
                end
            else
