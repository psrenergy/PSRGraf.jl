...
function PSRGraf.open(
    ...
    if !(0 < initial_stage <= STAGES_IN_YEAR[stage_type])
        error(
            "initial_stage must be between 1 and $(STAGES_IN_YEAR[stage_type]) for $stage_type files, got: $initial_stage",
        )
    end
...
function write_registry(
    ...
    blks_in_stage = blocks_in_stage(io, stage)

    if !(1 <= block <= blks_in_stage) # io.blocks
        error("block should be between '1' and '$blks_in_stage'")
    end
